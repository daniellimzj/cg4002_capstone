...
def main():
    for file in os.listdir(folder):
        if ([action for action in actions if (action in file)]) and "extracted" not in file:
            ...
            f2 = open(folder + "\\" + "extracted_" + file, "a+")
            lines = f.readlines()
            count = 0
            accX = []
            accY = []
            ...
            for line in lines:
                ...
                if count % 38 > 0:
                    ...
                    f2.write(str(meangyroX) + "," + str(rggyroX) + "," + str(vargyroX) + "," + str(medgyroX) + "," + str(masdgyroX) + ",")
                    f2.write(str(meangyroY) + "," + str(rggyroY) + "," + str(vargyroY) + "," + str(medgyroY) + "," + str(masdgyroY) + ",")
                    if count != len(lines):
                        f2.write(str(meangyroZ) + "," + str(rggyroZ) + "," + str(vargyroZ) + "," + str(medgyroZ) + "," + str(masdgyroZ) + "," + str(key[action]) + "\n")
                    else:
                        f2.write(str(meangyroZ) + "," + str(rggyroZ) + "," + str(vargyroZ) + "," + str(medgyroZ) + "," + str(masdgyroX) + "," + str(key[action]))
                    accX.clear()
                    accY.clear()
                    accZ.clear()
                    gyroX.clear()
    ...
    header_string = "Mean-accX,Range-accX,Var-accX,Median-accX,Masd-accX,Mean-accY,Range-accY,Var-accY,Median-accY,Masd-accY,Mean-accZ,Range-accZ,Var-accZ,Median-accZ,Masd-accZ,Mean-gyroX,Range-gyroX,Var-gyroX,Median-gyroX,Masd-gyroX,Mean-gyroY,Range-gyroY,Var-gyroY,Median-gyroY,Masd-gyroY,Mean-gyroZ,Range-gyroZ,Var-gyroZ,Median-gyroZ,Masd-gyroZ,label"
